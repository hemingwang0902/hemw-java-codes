...
public class TextComparer {
	...
	public static List<TextDiff> compare(String source, String compareTo) {
		...
		for (Point point : points) {
			...
			} else if (miarr[point.getX()][point.getY()] >= max && diarr[point.getX()][point.getY()] < minSetp) {
				minSetp = diarr[point.getX()][point.getY()];
			}
		}
		List<Point> willRemove = new ArrayList<Point>();
		for (Point point : points) {
			if (miarr[point.getX()][point.getY()] < max || diarr[point.getX()][point.getY()] > minSetp) {
				willRemove.add(point);
			}
		}
		points.removeAll(willRemove);
		willRemove.clear();
		List<TextDiff> diffs = calcDiffs(points.size() >= 1 ? points.get(0) : null, barr, miarr, diarr);
		points.clear();
		barr = null;
		miarr = null;
		diarr = null;
	 ...
	 * 计算异同
	 */
	private static List<TextDiff> calcDiffs(Point startPoint, boolean[][] barr, int[][] miarr, int[][] diarr) {
		if (startPoint == null)
			return new ArrayList<TextDiff>();
		...
		while (next != null) {
			if (!barr[startPoint.getX()][startPoint.getY()]) {
				if (startPoint.getX() != next.getX()) {
					diffs.add(new TextDiff(TextDiff.TYPE_INSERTED, startPoint.getX(), next.getX() - startPoint.getX()));
				}
				if (startPoint.getY() != next.getY()) {
					diffs.add(new TextDiff(TextDiff.TYPE_DELETED, startPoint.getY(), next.getY() - startPoint.getY()));
				}
			}
			startPoint = next;
		...
		if (startPoint.getX() != barr.length) {
			diffs.add(new TextDiff(TextDiff.TYPE_INSERTED, startPoint.getX(), barr.length - startPoint.getX()));
		}
		if (startPoint.getY() != barr[0].length) {
			diffs.add(new TextDiff(TextDiff.TYPE_DELETED, startPoint.getY(), barr[0].length - startPoint.getY()));
		}
		return diffs;
	}
	 ...
	 * 获取下一个匹配节点
	 */
	private static Point getNext(Point p, int[][] miarr, int[][] diarr, boolean[][] barr) {
	 ...
	 * 获取矩阵
	 */
	public static boolean[][] getCompareBooleanArray(String source, String compareTo) {
		boolean[][] comps;
	...
	public static int[][] getSetpPathArr(boolean[][] barr) {
		...
		for (int i = barr.length - 1; i >= 0; i--) {
			for (int j = barr[i].length - 1; j >= 0; j--) {
				...
				iarr[i][j] = v_n_1_1 > n_i_1_j ? v_n_1_1 > n_i_j_1 ? v_n_1_1 : n_i_j_1 : n_i_1_j > n_i_j_1 ? n_i_1_j
						: n_i_j_1;
			}
		}
		return iarr;
