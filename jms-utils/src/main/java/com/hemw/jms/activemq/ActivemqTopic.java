...
public class ActivemqTopic {
     ...
     * @throws JMSException
     */
    protected MessageProducer createProducer(Session session, String topicName) throws JMSException {
     ...
     * @throws JMSException
     */
    public void sendMessage(String topicName, String text) throws JMSException {
     ...
     * @throws JMSException
     */
    public void receiveMessage(String topicName) throws JMSException {
        Connection connection = createConnection();
        Session session = createSession(connection);
        Destination destination = session.createTopic(topicName);
    ...
    public static void main(String[] args) throws JMSException {
        ActivemqTopic test = new ActivemqTopic();
        String topicName = "topic.test";
        test.sendMessage(topicName, "这是一个测试消息，哈哈……");
//         test.receiveMessage(topicName);

        // 如果使用的是持久订阅（Durable Subscription）方式，则必须在建立连接（javax.jms.Connection）的时候设置ClientID
        // 同时，建立 javax.jms.MessageConsumer 的时候需使用 createDurableSubscriber() 方法，并为其指定一个 consumerName。
        // connection.setClientID(clientId);
