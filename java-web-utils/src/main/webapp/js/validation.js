...
function isMobile(str) {
    return str.match(/^(13\d{9})|(14\d{9})|(15\d{9})|(18\d{9})$/);
}

/** 判断一个号码是不是中国移动手机号码*/
function isChinaMobileNumber(str) {
    return str.match(/^(13[4-9]\d{8})|(15[01278-9]\d{8})|(18[2378]\d{8})|(147\d{8})$/);
};
...
// 判断一个字符串是不是合法的日期, 标准日期格式(yyyy-yy-dd)
function isDate(str) {
    return str.match(/^[1-2]\d{3}-(0?[1-9]|1[0-2])-(0?[1-9]|[12][0-9]|3[0-1])$/);
}

/**判断一个字符串是不是不是合法的Email地址*/
function isEmail(str) {
    return str.match(/^([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+@([0-9a-zA-Z]+.){1,2}(com|net|cn|com.cn)$/);
}
